		...
		if (array_key_exists('render', $args)) {
			if (in_array($args["render"],$this->render_options)) {
				if ($args["render"]=='default') {
					$this->render=$wgLoopFormulaDefaultRenderOption;
				} else {
	...
	function render() {
		...
		switch ($this->render) {
			...
			case 'icon':
				...
				//$return.='<div class="mediabox_typeicon"><img src="'.$wgStylePath .'/loop/images/media/type_formula.png"></div>';
				$return.='<div class="mediabox_typeicon"><div class="mediabox_typeicon_formula"></div></div>';
